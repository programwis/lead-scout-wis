...
- **`website`/`domain` เป็น `null` ได้ และการไม่มีเว็บ *ไม่ใช่* เหตุให้ตกรอบ** — ตัวตนธุรกิจมาจาก Maps
  เว็บมีไว้ enrich เท่านั้น · `confirmLeads()` บังคับแค่ `companyName` และ upsert ด้วย `domain`
  ถ้ามี ไม่มีก็ใช้ `companyName` + `address` · **ห้ามใช้ `{ domain: null }` เป็นตัวกรอง**
  จะไปแมตช์ lead ที่ไม่มี domain ทุกตัวแล้วเขียนทับกันเอง
- **`website` ต้องเป็นเว็บทางการเสมอ** — Google Maps ใส่เพจ Facebook ในช่อง website ได้
  ถ้าเอามาเป็น `domain` จะได้ `facebook.com` ซึ่งชน unique index กับทุกกิจการที่ทำแบบเดียวกัน
  (ทั้งฐานเหลือ lead ได้รายเดียว) จึงรับเป็น `website` เฉพาะที่ `classifySource()` ตอบว่า
  `official_website`/`government` **แต่ยัง crawl หน้านั้นเพื่อหาอีเมล** แล้วเก็บ URL เป็น social reference
- **`references` เก็บเฉพาะแหล่งที่พิสูจน์ได้ว่าเป็นของกิจการนี้** (`collectReferences()`)
  official_website 1 · directory 1 (หน้า Maps ของ cid ตัวเอง) · social ≤5 · government ≤2
  **ห้ามแก้ปัญหา reference รกด้วยการเพิ่ม blacklist** — ที่ตัดของเสียออกได้คือกติกาความสัมพันธ์:
  ไม่เก็บ directory จากลิงก์บนหน้าเว็บเลย (Google Drive/Forms/redirect หายหมด),
  government ต้องมีชื่อโดเมนของกิจการอยู่ใน URL (ไม่งั้นได้เว็บโรงพยาบาลอื่นติดมา),
  ถ้าหน้าที่ crawl เป็นเพจ social เก็บแค่ URL เพจนั้น (ลิงก์บนหน้า Facebook คือเมนูของ Facebook),
  และทิ้ง URL ที่เป็น path ย่อยของ reference อื่น (เก็บหน้าเพจ ไม่เก็บโพสต์)
- **`sourceType` อยู่ระดับ lead** (`"google_maps"`) คนละตัวกับ `SourceType` ที่จัดประเภท URL
...
11. **index ของ `domain` ต้องเป็น `sparse`** เพราะ lead ที่ไม่มีเว็บจะไม่มีฟิลด์นี้
   และ **ห้ามเขียน `domain: null` ลง DB** — sparse ข้ามเฉพาะเอกสารที่ "ไม่มีฟิลด์" ถ้าเขียน `null`
   ลงไปจะถูก index แล้วชน unique กันเอง `saveLead()` จึงไม่ใส่ key `domain` เลยเมื่อไม่มีค่า
   · ฐานข้อมูลที่สร้าง index เดิม (ไม่ sparse) ไว้แล้วต้อง drop index เก่าทิ้งก่อน mongoose ถึงจะสร้างใหม่ให้
